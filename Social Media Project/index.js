...
Dlete.forEach(deletee=>{
    deletee.addEventListener('click', ()=>{
        deletee.parentElement.style.display='none'
    })
})
...
document.querySelector('#profile-upload').addEventListener('change',()=>{
    document.querySelectorAll('#my-profile-picture img').forEach(AllMyProfiles=>{
        AllMyProfiles.src= URL.createObjectURL(document.querySelector('#profile-upload').files[0])
    })
})
